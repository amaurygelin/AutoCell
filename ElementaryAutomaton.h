...
class ElementaryAutomaton final: public Automaton{
    ...
    public:
        ElementaryAutomaton(short unsigned int r_num): Automaton(2,2), rule_num(r_num), rule_bits(num2bits(r_num)) {}
        ElementaryAutomaton(std::string r_bits): Automaton(2,2), rule_num(bits2num(r_bits)), rule_bits(r_bits) {}
        short unsigned int getRuleNum() const {return rule_num;}
        std::string getRuleBits() const {return rule_bits;}
        //void transition(const Grid& src, Grid& dest) const override final;
